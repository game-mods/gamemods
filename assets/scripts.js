...
{
	$.ajax({method: "POST", url: "ajax", dataType:rtype, data: "aid="+ aid + "&" + sendData, beforeSend: function(){ before();}, success: function(result){ after(result); }, error:function(result){ modal.msg("AJAX Error", "Ocorreu um erro durante a solicitação do ajax, por favor, atualize a página ou entre em contato com a equipe de administração."); $("body").css("cursor", "default"); }});
}
...
function must_login()
{
	modal.msg("Faça o login primeiro",'Você deve estar logado para fazer o upload, <a href="login">clique aqui</a> para fazer login.');
}
...
function open_image(url)
{
	...
	image.onerror = function(){ big_loader(false); close_img(); alert('Não foi possível carregar a imagem, tente atualizar a página.'); }
...
{
	call(3, form, btn, function(res){ modal.msg("Atualizar conta", res.msg + '<br><br><a onclick="modal.end();" class="btn">Ok</a>'); });
}
...
function change_password(form, btn)
{
	call(4, form, btn, function(res)
	{
		...
		modal.msg("Alterar a senha", res.msg);
	});
}
...
function update_picture(btn)
{
	...
	function(res)
	{
		...
		{
			modal.msg("Alterar imagem", res.msg);
		}
...
function upload_screenshot(input, maximum)
{
	...
	{
		modal.msg("Oops!", "O limite de imagens foi atingido.");
		$(input).val("");
	}
	else
	{
...
function image_preview_html(img_id, img_url)
{
	var h = '<div class="uimg ui_'+ img_id +'">';
		...
		h += '<input type="button" value="Remover" class="remove_btn" onclick="rip(\'' + img_id + '\');">';
		h += '</div>';
...
function pre_up_mod(input)
{
	...
	var h = '<div class="pre_up_box">';
	    h += 'Tem certeza de que deseja fazer upload do arquivo:<br>';
	    h += '<ul>';
		h += '<li>Nome: <b>'+ file.name +'</b></li>';
		h += '<li>Tamanho: <b>'+ bytesToSize(file.size) +'</b></li>';
		h += '<br><a class="btn up_btn" onclick="upload_mfile(\'#mod_file\');">Sim</a> <a class="btn" onclick="cancel_pre_up();">Não</a>';
		h += '</ul>';
		h += '</div>';
...
function upload_mfile(input)
{
	...
	function(res)
	{
		...
		if(res.OK)
		{
			...
			$(".file_res").html('Arquivo carregado: <a href="request/'+ res.file_id +'" target="_blank">'+ res.file_id +'</a> &nbsp;&nbsp;<small><a onclick="remove_up();">(remover)</a></small>').fadeIn();
		}
		else
		{
			modal.msg("Oops", res.msg);
...
function upload(form, btn)
{
	if($("#file_id").val().trim() == "")
	{
		modal.msg("Enviar um arquivo", "Você deve fazer upload de um arquivo.");
	}
	else if($(".gallery .uimg").length == 0)
	{
		modal.msg("Carregar uma captura de tela", "Você deve enviar pelo menos uma captura de tela.");
	}
	else
	{
		call(8, form, btn,
...
function update_mod(form, btn, up_type)
{
	switch(up_type)
	{
		case 1:
		   var AID = 12;
		   var success = 'Seu mod foi editado com sucesso!<br><br><a href="view/'+ mod_seo +'" class="btn up_btn">OK</a>';
		break;

		case 2:
		   var AID = 16;
		   var success = 'O mod foi atualizado com sucesso!<br><br><a class="btn up_btn" onclick="modal.end();">OK</a>';
		break;

		case 3:
		   var AID = 12;
		   var success = 'Seu mod foi editado com sucesso!<br><br><a href="my_uploads" class="btn up_btn">OK</a>';
		break;
	}
	...
	else
	{
		...
		function(res)
		{
			if(res.OK)
			{
				modal.start("Sucesso!", '<center><img src="assets/images/ok.png" width="100"><br><br>'+ success +'</center>', 370, 300, false);
			}
			else
			{
				modal.msg("Oops", res.msg);
...
{
	var btn_text = "Carregar mais comentários";
...
{
	var res = confirm("Deseja excluir o comentário?");
...
{
	call(13, form, btn, function(res){ modal.msg("Notas do administrador", res.msg); });
}

function save_web_settings(form, btn)
{
	call(14, form, btn, function(res){ modal.msg("Mensagem", res.msg); });
}

function save_ads(form, btn)
{
	call(15, form, btn, function(res){ modal.msg("Mensagem", res.msg); });
}

function update_user(form, btn)
{
	call(17, form, btn, function(res){ modal.msg("Mensagem", res.msg); });
}
...
function delete_mod(form, btn)
{
	call(18, form, btn, function(res)
	{
		if(res.OK)
		{
			$('.upload').html('<h2>Removido!</h2>O mod e seus comentários foram removidos com sucesso!<br><br><a class="btn" href="adm_mods">Ok &raquo;</a>');
		}
		else
		{
			modal.msg("Mensagem", res.msg);
		}
	});
}
...
function wait()
{
	modal.msg("Sob revisão", "Seu mod está em análise.<br>Isso pode levar algumas horas!");
}
...
function upload_image(aid, from, before, after)
{
	...
	if(file == null)
	{
		modal.msg("Error", "O arquivo não pode estar vazio.");
		$(from).val("");
	}
	else if(!(/\.(jpg|jpeg|png)$/i).test(file.name))
	{
		modal.msg("Error", "Você deve selecionar um tipo de imagem válido (JPEG, JPG, PNG).");
		$(from).val("");
	}
	else if(file.size > mbtob(MFS_IMAGE))
	{
		modal.msg("Error", "O tamanho do seu arquivo de imagem não pode ser maior que "+ MFS_IMAGE +" MB.");
		$(from).val("");
	}
	else
	{
		...
		reader.onloadend = function(){
			...
			image.onload = function(){
				...
				error: function(e){ $(from).val(""); modal.msg("Error", "Ocorreu um erro durante o upload da imagem, atualize a página."); } });
			}
		}
...
function upload_file(aid, from, before, after)
{
	...
	{
		modal.msg("Error", "Você deve selecionar um arquivo válido (RAR, ZIP, 7Z).");
		$(from).val("");
	}
	else if(file.size > mbtob(MFS_UPLOAD))
	{
		modal.msg("Error", "O tamanho do arquivo não pode ser maior que "+ MFS_UPLOAD +" MB.");
		$(from).val("");
	}
	else
	{
		...
		$.ajax({url : "ajax", type : "POST", data :sendData, processData: false, contentType: false, dataType:"json",
		   ...
		   error:function(e)
		   {
			   $(from).val(""); modal.msg("Error", "Ocorreu um erro durante o upload do arquivo, atualize a página.");
		   }
		});
	}
}
