...
class mods{
	...
	function load_comments($mod_id, $last_id)
	{
		...
		if($query->num_rows != 0)
		{
			...
			while($data = $query->fetch_assoc())
			{
				...
				if($data["by_uid"] == UID || RANK == 2){ $infos = '<a onclick="remove_comment('.$data["cid"].')">remover</a> &middot; '.$time; } else { $infos = $time; }
	...
	function load_my_uploads()
	{
		...
		if($query->num_rows == 0)
		{
			if($this->page_id == 1)
			{
				return '<font color="#666">Você não tem mods carregados.<br><br><a href="upload" class="btn up_btn">Carregar meu primeiro Mod &raquo;</a></font>';
			}
			else
			{
				return 'Não há mods para ver nesta página ('.$this->page_id.').<br><br><a href="my_uploads" class="btn">&laquo; Voltar</a>';
			}
		}
		else
		{
	...
	function load_list($nm = "")
	{
		...
		if($query->num_rows == 0)
		{
			...
			{
				return 'There are no mods to see at this page ('.$this->page_id.').<br><br><a onclick="history.go(-1);" class="btn">&laquo; Go back</a>';
			}
		}
		else
		{
		...
		if($query->num_rows == 1)
		{
			...
			if(dbquery('INSERT INTO comments (by_uid, mod_id, comment, visible, unixtime) VALUES ('.UID.', '.$mod_id.', "'.$comment.'", 1, "'.time().'")'))
			{
				...
				if(dbquery('UPDATE mods SET total_comments = (total_comments + 1) WHERE mid = '.$mod_id))
				{
					...
					$tpl->set_param("infos", '<a onclick="remove_comment('.$last_id.')">remover</a> &middot; agora mesmo');
