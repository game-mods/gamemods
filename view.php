...
if(isset($_GET["seo"]))
{
	...
	if($mods->exists && $mods->status == 1)
	{
		...
		if(RANK == 2){ $edit_btn = '<a class="btn edit_btn" href="adm_edit?mid='.$mods->id.'">Editar Mod (Painel)</a>'; }
		// check if current user is owner, if yes, show edit button
		elseif($mods->user_id == UID){ $edit_btn = '<a class="btn edit_btn" href="edit/'.$seo.'">Editar mod</a>'; }

		// show comments area ? ( if user is online, yes! )
		$com_style = (ONLINE)?'block':'none';
		$join_con  = (ONLINE)?'':'<div class="wbox join">Junte-se à conversa! <a href="login">Faça login</a> ou <a href="signup">registe-se</a> para uma conta poder comentar.</div>';

		// last edition html
		$last_edited = (empty($mods->last_edited))?'':'<tr><td>Última edição:</td><td>'.date('d/m/y', strtotime($mods->last_edited)).'</td></tr>';
		...
		$tpl->set_param("up_date", date('d/m/y', strtotime($mods->unix)));
		...
		$lmc = ($mods->comments > $mods->cpl)?'<input type="button" id="lmc_btn" onclick="lm_comments('.$mods->id.', this);" value="Carregar mais comentários">':'';
